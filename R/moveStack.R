	  ...
	  definition = function(x,forceTz=NULL,...){
	    ...
	    if(any(duplicated(unlist(lapply(lapply(x, slot, "idData"), rownames)))))
		  ...
		  unUsedList<-mapply(function(x,i,j){if(length(i)!=0){x[i,j]}else{x}}, 
				     unUsedList,
				     i=lapply(lapply(unUsedList, slot,'timestampsUnUsedRecords'), 
				              function(x){if(is.null(x)){return(NULL)}else{return(order(x))}}), 
		  ...
		  sensorLevels<-unique(unlist(c(lapply(lapply(x, slot, 'sensor'), levels), 
		  ...
		  unUsed<-new(".unUsedRecordsStack",
			      ...
			      sensorUnUsedRecords=factor(unlist(lapply(
			        ...
			        as.character)), 
			        levels=sensorLevels),
			      trackIdUnUsedRecords=factor(unlist(mapply(rep, rownames(IDDATA), 
