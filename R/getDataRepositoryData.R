...
setMethod(
  ...
  definition =
    function(x, ...) {
      ...
      fileUrl <-
        unlist(lapply(fileUrl, function(url, x)
          modify_url(url, path = x), url = parse_url(metaData$url)))
    ...
    naFilledCols <-
      ...
      allLocData <-
        allLocData[order(as.character(allLocData$individual.local.identifier),
      ...
      idData <-
        merge(refData, allLocData[!duplicated(allLocData$`individual.local.identifier`), c("individual.local.identifier", toMove), drop =
                                    F], by.x = "animal.id", by.y = "individual.local.identifier")
