...
setAs(".MoveTrackSingleBurst","data.frame", function(from){t<-burstId(from)
      t[n.locs(from)]<-NA
...
setAs(".unUsedRecordsStack","data.frame", function(from){
	      return(data.frame(
				as(as(from,'.unUsedRecords'),'data.frame'), trackId=from@trackIdUnUsedRecords))
})
setAs(".unUsedRecords","data.frame", function(from){
	      return(
		     data.frame(
				from@dataUnUsedRecords, sensor=from@sensorUnUsedRecords, timestamps=from@timestampsUnUsedRecords)
		     )
})
setMethod("as.data.frame", ".unUsedRecords",
	  function (x)
	  {
		  as(x, "data.frame")
	  }
	  )
setMethod("as.data.frame", ".unUsedRecordsStack",
	  function (x)
	  {
		  as(x, "data.frame")
	  }
	  )
setMethod("as.data.frame", "dBMvariance",
	  function (x)
	  {
		  as(x, "data.frame")
	  }
	  )
setMethod("as.data.frame", "dBMvarianceStack",
	  function (x)
	  {
		  as(x, "data.frame")
	  }
	  )
setMethod("as.data.frame", "Move",
	  function (x)
	  {
		  as(x, "data.frame")
	  }
	  )
setMethod("as.data.frame", "MoveStack",
	  function (x)
	  {
		  as(x, "data.frame")
	  }
	  )
setMethod("as.data.frame", "MoveBurst",
	  function (x)
	  {
		  as(x, "data.frame")
	  }
