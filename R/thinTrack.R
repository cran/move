...
filterThinFun<-memoise(function(x, d, t, selType=c('closest','first','all')){
			       ...
			       m<-m[switch(selType[1], # select solutions
					   closest=which.min(unlist(lapply(unlist(recursive=F,lapply(m, attr, 't')), function(xx){a<-abs(diff(c(x[1],xx))-d); sum(a[a<=t])}))), # check if there is a way to do this without adding an n^2 not sure how bad this problem is 
					   first=1,
					   all=T
					   )]
