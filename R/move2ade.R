...
setAs("telemetry","Move", function(from){
  if(class(from)=="telemetry"){
    mv <- move(x= from$x, y= from$y,
               ...
               animal= if(is.null(from@info$identity)){"unnamed"}else{as.character(from@info$identity)}
    )
    if("VAR.xy" %in% names(from)){mv$error.loc.mts <- sqrt(from$VAR.xy)}
    return(mv)
  }
})
  ...
  if(unique(unlist(lapply(from, class)))=="telemetry"){
    stk <- moveStack(lapply(from, function(x){
      mv <- move(x= x$x, y= x$y,
                 ...
                 proj= if(is.null(x@info$projection)){as.character(NA)}else{x@info$projection},
                 animal= if(is.null(x@info$identity)){"unnamed"}else{as.character(x@info$identity)}
      )
      if("VAR.xy" %in% names(x)){mv$error.loc.mts <- sqrt(x$VAR.xy)}
      return(mv)
    }), forceTz=from[[1]]@info$timezone)
    return(stk)
  }else{stop("This list cannot be transformed into a movestack object, only a list of objects of class \"telemetry\" can be transformed")}
})
  ...
  if(class(from)=="binClstStck"){
    mv <-  moveStack(lapply(1:length(from@bCS), function(i){
      move(x= from@bCS[[i]]@pth$lon,
           ...
           proj= if(from@bCS[[i]]@tracks@proj4string@projargs==""){as.character(NA)}else{from@bCS[[i]]@tracks@proj4string@projargs},
           animal= paste0("unnamed",i))
    }), forceTz=if(is.null(attr(from@bCS[[1]]@pth$dTm[1],"tzone"))){Sys.timezone()}else{attr(from@bCS[[1]]@pth$dTm[1],"tzone")})
    return(mv)
  }
})
