...
setMethod("emd", signature = c(x = "RasterLayer", y = "RasterLayer"),
          function(x, y, ...)
          {
...
setMethod("emd", signature = c(x = "SpatialPoints", y = "SpatialPoints"),
          function(x,
                   y,
                   gc = FALSE,
                   threshold = NULL,
                   ...)
          {
            ...
            if (!isTRUE(all.equal(sum(xv), sum(yv))))
              warning(paste("rasters dont have equal mass, delta:", sum(xv) -
                              sum(yv)))
            if (!isTRUE(all.equal(sum(xv), 1)))
              warning("raster does not represent probability surface")
            res <- 1
            fun <- "emdR"
            if (!is.null(threshold))
            {
              if (!isTRUE(all.equal(coordinates(y), coordinates(x), check.attributes =
                                    F))) {
                stop("raster definitions are unequal this does not work with the fast emd")
              }
              fun <- "emdR_gd"
              s <- !(xv == 0 & yv == 0)
              xv <- xv[s]
...
setMethod("emd", signature = c(x = "RasterStackBrick", y = "missing"),
          function(x, y, ...)
          {
...
setMethod("emd", signature = c(x = "RasterStackBrick", y = "RasterStackBrick"),
          function(x, y, ...)
          {
