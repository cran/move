...
setGeneric("getDuplicatedTimestamps", function(x,  ...){standardGeneric("getDuplicatedTimestamps")})
...
setMethod("getDuplicatedTimestamps", 
	  ...
	  definition=function(x,..., onlyVisible=TRUE){
		  ...
		  callGeneric(x=factor(x$individual.local.identifier), 
			      timestamps= as.POSIXct(strptime(as.character(x$timestamp), 
							      format = "%Y-%m-%d %H:%M:%OS",tz="UTC"), 
						     tz="UTC"), # need to make character out of it to ensure milli seconds are considerd
...
setMethod("getDuplicatedTimestamps", 
	  ...
	  definition=function(x,timestamps,sensorType, visible=NULL, ...){
		  ...
		  stopifnot(length(x)==length(sensorType))
		  ...
		  if(anyDuplicated(df)){
			 s<-duplicated(df)|duplicated(df, fromLast=T) 
			 return(split((1:nrow(df))[s], df[s,c("i","t","s")], drop=T, sep = "|"))
