...
test_that("emd over simple distances", {
  ...
  expect_equal(as.numeric(emd(r3, r4)), .25, tolerance = 1e-6)
  expect_equal(
    as.numeric(emd(r4, r1)),
    .5 * .5 + sqrt(2 * .5^2) * .5,
    tolerance = 1e-6
  )
})
...
test_that("emd thresholds simple cases", {
  ...
  expect_equal(
    as.numeric(emd(r4, r1, th = 1)),
    .5 * .5 + sqrt(2 * .5^2) * .5,
    tolerance = 1e-6
  )
  expect_equal(
    as.numeric(emd(r4, r1, th = .6)),
    .5 * .5 + .6 * .5,
    tolerance = 1e-6
  )
...
test_that("emd th complex", {
  ...
  r2 <- r2 / cellStats(r2, "sum")
  expect_equal(
    emd(r1, r2, th = 12) |> `attr<-`('class', NULL),
    emd(r2, r1) |> `attr<-`('class', NULL),
    tolerance = 9e-5
  )
  expect_equal(
    as.numeric(emd(r1, r2, th = .01)),
    sum(values(abs(r1 - r2)) / 2) * .01,
    tolerance = 9e-6
  )
  expect_equal(
    as.numeric(emd(r1, r2, th = .021)),
...
test_that("emd with gc and threshold unequal rasters", {
  ...
  expect_equal(
    as.numeric(emd(r1, r2, gc = T)),
    distHaversine(c(.5, .5), c(1.5, 1.5)) / 1000
  )
...
test_that("emd with gc and threshold equal rasters", {
  ...
  expect_equal(
    as.numeric(emd(r1, r2, gc = T, th = 1000)),
    distHaversine(c(.5, .5), c(1.5, 1.5)) /
      1000
  )
...
test_that("large threshold is same as full and order does not matter", {
  set.seed(23423)
  s1 <- SpatialPointsDataFrame(
    matrix(ncol = 2, rnorm(20)),
    data = data.frame(a = rep(.1, 10))
  )
  s2 <- s1
  s2$a <- 1:10 / 55

  expect_equal(
    e <- (emd(s1, s2) |> `attr<-`('class', NULL)),
    emd(s1, s2, threshold = 100) |> `attr<-`('class', NULL)
  )
  expect_equal(
    e,
    emd(s1[sample(1:10), ], s2[sample(1:10), ]) |> `attr<-`('class', NULL)
  )
  expect_equal(
    e,
    emd(s1[sample(1:10), ], s2[sample(1:10), ]) |> `attr<-`('class', NULL),
...
test_that("emd for raster brick", {
  ...
  expect_equivalent(
    emd(s, ss, gc = T),
    distm(
      rasterToPoints(s)[, 1:2],
      rasterToPoints(ss)[, 1:2],
      fun = distHaversine
    ) /
      1000
  )
...
test_that("emd with spatial objects, compare spdf and sp", {
  ...
  ad <- SpatialPointsDataFrame(
    a,
    data = data.frame(w = rep(1 / length(a), length(a)))
  )
  b <- SpatialPoints(matrix(rnorm(34), ncol = 2))
  bd <- SpatialPointsDataFrame(
    b,
    data = data.frame(w = rep(1 / length(b), length(b)))
  )
  d <- SpatialPoints(matrix(rnorm(38), ncol = 2))
  dd <- SpatialPointsDataFrame(
    d,
    data = data.frame(w = rep(1 / length(d), length(d)))
  )
  expect_equal(
    emd(a, b) |> `attr<-`('class', NULL),
    emd(ad, bd) |> `attr<-`('class', NULL)
  )
  expect_equal(
    emd(a, d) |> `attr<-`('class', NULL),
    emd(ad, dd) |> `attr<-`('class', NULL)
  )
})
