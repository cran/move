...
If you do not have a Movebank account, you can register at www.movebank.org or download csv files of publicly available studies directly from Movebank and read them into the Move package as described in the help file of the \texttt{move} function.
...
\subsection{Get all reference data of a study}
...
In case the study contains duplicated timestamps, you can set the argument\\ \texttt{removeDuplicatedTimestamps=TRUE}. It is strongly advised not to use this option because there is no control over which records are removed.

<<removeDuplicatedTimestamps, results=hide, eval=F>>=
bci_ocelot <- getMovebankData(study="BCI Ocelot", login=loginStored,
                              removeDuplicatedTimestamps=TRUE)
@
The presence of duplicated timestamps can be due do many different reasons. There are several options to deal with this. One method is for a data manager of the study to review and flag the duplicates in Movebank, \href{https://www.movebank.org/node/42#mark_outliers}{either manually} or \href{https://www.movebank.org/node/33529}{using filters}, including a duplicate filter that flags duplicate records based on user-selected attributes.
Another possibility is to download the data directly from Movebank, or via the \fct{getMovebank} function, and then use the function \fct{getDuplicatedTimestamps} to find the duplicated timestamps and to decide which one to remove before creating a move or movestack object using the \fct{move} function. These flagged duplicated records can be also marked as outliers in Movebank, by \href{https://www.movebank.org/node/2874}{adding an attribute} like e.g. "manually\_marked\_outlier" to the study.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section[Download data from the Movebank Data Repository as a 'move/moveStack' object]{Download data from the Movebank Data Repository\newline as a 'move/moveStack' object}

This function downloads data from the \href{https://www.movebank.org/node/15294}{'Movebank Data Repository'}. It returns a \texttt{move} object (if only one individual is included) or a \texttt{moveStack} (if several individuals are included). If data of non-location sensors are included in the data set, these will be stored in the 'unUsedRecords' slot of the move object.
